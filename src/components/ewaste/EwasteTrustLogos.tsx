...
export function EwasteTrustLogos() {
  return (
      ...
      <div className="w-full flex-col px-[67px] max-w-[1920px]">
        ...
        <div className="w-full border border-gray-200 rounded-sm grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 overflow-hidden">
          {gridLogos.map((logo, idx) => {
            // Determine border styles based on position to avoid double borders.
            // In a 6-col grid, the rightmost column doesn't need a right border.
            // The bottom row doesn't need a bottom border.
            // Tailwinds `border-r` and `border-b` apply easily if we aren't at the edge.
